...
int	main(void)
{
	...
	check("ft_memchr finds visible byte 'c'",
		ft_memchr(buf, 'c', sizeof(buf)) == memchr(buf, 'c', sizeof(buf)));
	check("ft_memchr finds byte after embedded NUL",
		ft_memchr(buf, 'e', sizeof(buf)) == memchr(buf, 'e', sizeof(buf)));
	check("ft_memchr returns NULL for missing byte",
		ft_memchr(buf, 'z', sizeof(buf)) == NULL);
	check("ft_memchr with length 0 returns NULL",
		ft_memchr(buf, 'a', 0) == NULL);
	print_result();
	return (g_fails != 0);
}
*/
